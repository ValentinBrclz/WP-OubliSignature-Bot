...
var WikimediaStream = require('wikimedia-stream'),
	...
	Bot = require('nodemw'),
	client = new Bot(__dirname + '/config.json'),
	async = require('async'),
	_ = require('lodash'),
...
try {
	...
	// Re-init test page
	rackSandbox(client.getConfig('testpage'), function(err, data) {});
		...
		if (isDiscussion) {
			...
			isUserElligible(data.user, data.flags, function(err, isUserElligible) {
				...
				if (isUserElligible) {
					...
					log(id + ' - Checking diff on [[' + data.page + ']] by ' + data.user);
							...
							async.each(addedLines, function(line, callback) {
								...
								if (line.length > 5) {
									if (isUnsignedComment(trimmedline, data.page, data.comment, data.user, content)) {
										log(id + ' - Message unsigned: ' + data.url);
										...
										delay(120000, function() {
											client.logIn(function(err) {
												...
												addSignature(data.page, trimmedline, id, data.user, section, function(err) {
													if (err) {
														addErrorLine(data.page, data.url, data.user, 'addSignature()', err);
														return;
													}
...
function getAddedLinesFromUrl(url, page, callback) {
	...
	/**
	 * @param {Object} info
	 * @param {Object} info.pages
	 * @param {Array} info.revisions
	 */
	client.api.call(params, function(err, info) {
		if (err) {
			log('Error: ' + err + '[' + info.toString() + ']');
		}
...
/**
 * Get the timestamp of a revision based on its id
 * @param {number} id - the id from which we want the timestamp
 * @param callback
 */
function getTimestampFromId(id, callback) {
	// URL: w/api.php?action=query&format=json&prop=revisions&revids=161830895&rvprop=timestamp
	var params = {
		action: 'query',
		prop: 'revisions',
		rvprop: 'timestamp',
	 ...
	 * @param {String} info.properties.name
	 */
	log(params);
	client.api.call(params, function(err, info) {
		if (err) {
		...
		try {
			if (Object.values(info.pages)[0].revisions[0].timestamp !== undefined) {
				callback(null, Object.values(info.pages)[0].revisions[0].timestamp);
				return;
			}
		}
		catch (err) {
 ...
 * @param callback
 */
function rackSandbox(page, callback) {
	client.logIn(function(err) {
		if (err) {
			return;
		}
...
function addErrorLine(page, url, user, type, error) {
	...
	client.getArticle('Utilisateur:Signature manquante (bot)/Journal', function(err, content) {
		...
		client.edit(
			'Utilisateur:Signature manquante (bot)/Journal',
			newcontent,
			'Ajout d\'une erreur',
			true,
			function(err) {
...
function addMsgOnUserPage(user, page, url, callback) {
	client.getArticle('Discussion utilisateur:' + Html5Entities.decode(user), function(err, content) {
		...
		var ip = isIPaddress(user) ? '-IP' : '',
			newcontent = content +
				'\n\n{{subst:User:Signature manquante (bot)/Modèle:SignezSVP' + ip +
				'|' + page +
				'|2=' + url +
				'|3=' + new Date().getDayHash() +
				'|4=' + user +
				'}}\n';
		...
		client.edit(
			...
			function(err, data) {
				if (err) {
					// Handle flow-board error
					if (err.toString().contains('flow-board')) {
						addMsgOnUserPageFlow(user, page, url, function(err, data) {
							if (err) {
								callback(err);
								log(data);
							}
...
/**
 * Add a message to the discussion page with flow-model of a user
 * @param {string} user - the user to warn
 * @param {string} page - the related page
 * @param {string} url - the diff url
 * @param callback
 */
function addMsgOnUserPageFlow(user, page, url, callback) {
	var content = '{{subst:User:Signature manquante (bot)/Modèle:SignezSVP' +
		'|' + page +
		'|2=' + url +
		'|3=' + new Date().getDayHash() +
		'|4=' + user +
		'|flow=yes}}',
		topic = 'Signature manquante sur [[' + page + ']]';
 ...
 * @param callback
 */
function addSignature(page, line, id, user, section, callback) {
	...
	client.getArticle(page, function(err, content) {
		...
		// Handle IP / User differences
		var ip, userlink;
		if (isIPaddress(user)) {
			ip = '|ip=oui';
			userlink = '[[Spécial:Contributions/' + user + '|' + user + ']]';
		} else {
			ip = '';
		...
		// Variables
		var unsigned = ' {{non signé|' + user + '|~~~~~|' + id + ip + '}}',
		...
		// Get timestamp
		getTimestampFromId(id, function(err, timestamp) {
			if (err) {
				callback(err);
				return;
			}
			...
			client.doEdit(
				'edit',
				page,
				section + 'Ajout de la [[Aide:Signature/résumé|signature]] au message de ' + userlink + ' - plus d\'informations sur la [[Aide:Signature/résumé|page d\'aide sur la signature]]',
				params,
				function(err, data) {
					if (err) {
						callback(err);
...
function isUnsignedComment(line, title, summary, username, content) {
	var regex,
		...
		unsignedRegex = new RegExp('({{u\'?\\||\\[\\[(:?[a-z]+:)*(Utilisat(eur|rice)|User|User( |_)talk|Spécial:Contributions/|Discussion( |_)Utilisat(eur|rice)): *)' + userregexp, 'i');
...
function isAutopatrolled(username, callback) {
	...
	/**
	 * @param {Object} info
	 * @param {Array} info.users
	 * @param {Array} info.users.groups
	 */
	client.api.call(params, function(err, info) {
...
function hasAlreadyBeenMessagedToday(content) {
	if (!content.includes('<!--SIGNATUREBOT '))
		return false;

	var matches = content.match(/<!--SIGNATUREBOT ([0-9]+) -->/g);

	return (matches !== null && matches[matches.length - 1][1] === new Date().getDayHash());
}
...
function delay(time, callback) {
	setTimeout(function() { callback(null, null) } , time);
}
...
/**
 * Get the date in a day-of-theyear+year format
 * @returns {string}
 */
Date.prototype.getDayHash = function() {
	/**
	 * @type {Date}
	 */
	var d = this;
...
function log(text) {
	...
	console.log('[' + time + '] ' + text.toString());
}
