...
function addSignature(page, line, id, user, section, callback) {
	...
	client.getArticle(page, function(err, content) {
		...
		client.edit(
			page,
			newcontent,
			section + 'Ajout de la [[Aide:Signature/résumé|signature manquante]] au message de ' + userlink + '. (Plus d\'informations : [[Aide:Signature/résumé|page d\'aide sur la signature]])',
			false,
			function(err, data) {
				if (err) {
					callback(err);
