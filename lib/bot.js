								...
								if (line.length > 5) {
									if (isUnsignedComment(trimmedline, data.page, data.comment, data.user, content)) {
										...
										client.logIn(function (err) {
											...
											addLogLine(data.page, data.url, data.user, function (err) {
												if (err) {
													addErrorLine(data.page, data.url, data.user, "addLogLine()", err);
												}
 ...
 * @param callback
 */
function addLogLine(page, url, user, callback) {
	client.getArticle("Utilisateur:Signature manquante (bot)/Journal", function (err, content) {
		...
		var row = " |----\n | [" + url + " ~~~~~]  || [[" + page + "]]  || {{u|" + user + "}}\n |}<!-- END DETECTIONS -->",
			newcontent = content.replace(" |}<!-- END DETECTIONS -->", row);

		client.edit("Utilisateur:Signature manquante (bot)/Journal", newcontent, "Ajout d'une ligne pour [[User:" + user + "]] sur [[" + page + "]]", function (err) {
			if (err) {
				callback(err);
				return;
			}
...
function addMsgOnUserPage(user, page, url, callback) {
	client.getArticle("Discussion utilisateur:" + Html5Entities.decode(user), function (err, content) {
		...
		if (!isMessageAllowedInContent(content))
			return;

		if (hasAlreadyBeenMessagedToday(content))
			return;

		var newcontent = content + "\n\n{{subst:User:Signature manquante (bot)/Modèle:SignezSVP|" + page + "|2=" + url + "|3=" + getStringDate() + "}}\n";
...
function signLine(page, line, id, user, callback) {
	...
	client.getArticle(page, function (err, content) {
		...
		client.edit(
			page,
			newcontent,
			"[[Aide:Signature/résumé|Signature manquante]]",
			function (err, data) {
				if (err) {
					callback(err);
					return;
...
function isUnsignedComment(line, title, summary, username, content) {
	var regex,
		...
		unsigned_regex = new RegExp("({{u'?\\||\\[\\[(Utilisat(eur|rice)|User|User talk|Spécial:Contributions/|Discussion Utilisat(eur|rice)):)" + userregexp, "i");
	...
	return (!unsigned_regex.test(Html5Entities.decode(line)));
}
 ...
 * @return {boolean}
 */
function isMessageAllowedInContent(content) {
	if (content === undefined)
		return false;

	return !(content.indexOf("{{nobots}}") != -1 || /\{\{bots\|[^}]*optout=(all|[^}]*SignezSVP)[^}]*}}/i.test(content));
}
...
function hasAlreadyBeenMessagedToday(content) {
	...
	return (content.match(/<!--S([0-9]{6})-->/)[1] == getStringDate());
}
