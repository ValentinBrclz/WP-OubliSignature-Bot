	...
	client.getArticle(page, function(err, content) {
		...
		getTimestampFromId(id, function(err, timestamp) {
			...
			client.doEdit(
				'edit',
				page,
				section + 'Ajout de la [[Aide:Signature/résumé|signature manquante]] au message de ' + userlink + ' - plus d\'informations sur la [[Aide:Signature/résumé|page d\'aide sur la signature]]',
				params,
				function(err, data) {
					if (err) {
						callback(err);
